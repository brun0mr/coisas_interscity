...
def criar_or_update_capabilitie () :
    ...
    if opcao == "1" :
       r = requests.post("http://34.95.144.147:8000/catalog/capabilities", data=data)
       print("capability criada!")
        
    else:
       nome_cap = input("entre com o nome da capability que deseja atualizar\n")
       r = requests.put(f'http://34.95.144.147:8000/catalog/capabilities/{nome_cap}', data=data)
...
def criar_resource () :
    ...
    data = '{"data": {"description": "sensorsala","capabilities": ["temperatura"],"status": "active","lat": 0,"lon": 0}}'
...
def atualizar_resource():
    ...
    r = requests.put(f"http://34.95.144.147:8000/adaptor/resources/{uuid}", data=data)
    print(r)
    print(r.content)
...
def actuator_command() :
    ...
    r = requests.post("http://34.95.144.147:8000/actuator/commands", data=data)
